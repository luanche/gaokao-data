...
OUT_DIR = os.path.join(os.path.dirname(DATA_DIR), ".tmp")
...
def main():
    ...
    rank_2026 = None
    for r in dist_2026:
        if r.get('科类') == '物理类' and r.get('分数') == '497':
    ...
    for y in [2025, 2024, 2023]:
        best = None
        for r in dist_old:
            if r.get('年份') == y and r.get('科类') == '物理类' and r.get('批次') == '本科批':
                cum = r.get('累计人数')
    ...
    for item in comb:
        ...
        major = item.get('专业名称')
        year = item.get('年份')
        if item.get('科类') == '物理' and item.get('批次') == '本科批':
            key = (code, major, year)
    ...
    for item in major_scores:
        ...
        major = item.get('专业名称')
        year = item.get('年份')
        if item.get('科类') == '物理类' and item.get('批次') == '本科批':
            key = (code, major, year)
